...
function sendResponse(SoapBackendResponse backendResponse) returns http:Response|ErrorResponse {
    ...
    // FIX 2 & 3: use getHeaderNames() then getHeaders() to avoid type mismatch
    string[] headerNames = backendResponse.httpResponse.getHeaderNames();
    foreach string headerName in headerNames {
        string[]|http:HeaderNotFoundError headerValues = backendResponse.httpResponse.getHeaders(headerName);
        if headerValues is string[] {
            foreach string headerValue in headerValues {
                finalResponse.addHeader(headerName, headerValue);
            }
        }
    }
...
// FIX 1: build http:Request manually instead of passing map<string|string[]> as post() argument
function soapBackendInvocation(string endpoint, xml payload, string soapAction, map<string|string[]> headers) returns SoapBackendResponse|error {
    ...
    req.setTextPayload(payload.toString(), "text/xml; charset=utf-8");
    ...
    http:Response resp = check httpClient->post("/", req);
